...
setwd("C:/RAHIM/MMA -QUEEN'S/Marketing Analytics - MMA 831/Airbnb")

library(lubridate)
library(xgboost)
library(caret)

train_usrs <- read.csv("train_users_2.csv", stringsAsFactors = FALSE)
#test_usrs <- read.csv("test_users.csv", stringsAsFactors = FALSE)
...
session_info <- read.csv("sessions2.csv", stringsAsFactors = FALSE)
...
#write.csv(all_usrs, "all_usrs.csv", row.names = FALSE)
...
write.csv(predictions, "sub_u26a_raw_prob.csv", quote=FALSE, row.names = FALSE)
...
predictions$TestId <- test$id
predictions <- predictions[,c("TestId", names(predictions)[1:12])]
write.csv(predictions, "sub_u26a_raw_rank.csv", quote=FALSE, row.names = FALSE)
...
write.csv(submission, "sub_u26a_rank.csv", quote=FALSE, row.names = FALSE)
